...
import com.maxiluna.studentmanagement.domain.usecases.classRecord.UpdateClassRecordUseCase;
...
@Service
public class UpdateClassRecordService implements UpdateClassRecordUseCase {
    @Autowired
    private JpaClassRecordRepository classRecordRepository;

    @Override
    public void execute(Long classRecordId, ClassRecord updatedClassRecord) {
        if (classRecordId <= 0) {
        ...
        try {
            ...
            classRecordJpa.setTopic(updatedClassRecord.getTopic());
            classRecordJpa.setActivities(updatedClassRecord.getActivities());
            classRecordJpa.setDate(updatedClassRecord.getDate());

            classRecordRepository.save(classRecordJpa);
        } catch (DataAccessException ex) {
            throw new DatabaseErrorException("Error updating class record: " + ex.getMessage());
        }
    }
}
